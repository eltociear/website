<!-- src/routes/frameworks/guides/bioregional-compass/library/the-learning-journey/+page.svelte -->
<script>
  ...
  // Component state
  let mounted = false;
  let activeResource = 'pilot-project-sequences'; // Start with pilot projects
  let expandedOverviews = new Set(['pilot-project-sequences']); // Default expand first
  ...
  function getCategoryColor(category) {
    switch (category) {
      case 'implementation': return '#4169E1';
      case 'community': return '#228B22';
      case 'development': return '#9932CC';
      case 'preservation': return '#8B4513';
      default: return '#6b7280';
    }
  }
...
<svelte:head>
  <title>{getTextWithFallback('guidesBioregionalCompassLibrary.learningJourney.title', 'The Learning Journey - Bioregional Compass')}</title>
  <meta name="description" content="{getTextWithFallback('guidesBioregionalCompassLibrary.learningJourney.description', 'Educational resources, support networks, and skill development for BAZ formation')}" />
</svelte:head>
...
{#if mounted}
  ...
  <div class="breadcrumb-nav">
    <nav class="breadcrumb-container">
      ...
      <span class="breadcrumb-separator">›</span>
      <span class="breadcrumb-current">The Learning Journey</span>
    </nav>
  </div>
  ...
  <div class="page-container">
    ...
    <header class="page-header">
      ...
      <p class="page-subtitle">
        Educational resources, support networks, and skill development for successful BAZ formation
      </p>
    </header>
    ...
    <div class="content-layout">
      ...
      <aside class="resource-nav">
        <div class="nav-header">
          <h2 class="nav-title">Learning Resources</h2>
          <p class="nav-subtitle">{data.totalResources} development tools</p>
        </div>
                ...
                <div class="resource-overview" transition:slide={{ duration: 200 }}>
                  ...
                  <div class="overview-meta">
                    ...
                    <div class="meta-item">
                      ...
                      <span class="meta-value">Complete Guide</span>
                    </div>
                  </div>
      ...
      <main class="main-content">
        ...
        <div class="resource-content">
          {#if currentResource}
            ...
            <!-- Resource Navigation -->
            {#if currentResource}
              {@const currentIndex = data.resources.findIndex(r => r.id === activeResource)}
              <div class="resource-navigation">
                {#if currentIndex > 0}
                  <button 
                    class="nav-btn prev-btn" 
                    on:click={() => setActiveResource(data.resources[currentIndex - 1].id)}
                  >
                    ← Previous: {data.resources[currentIndex - 1].title}
                  </button>
                {/if}
                ...
                {#if currentIndex < data.resources.length - 1}
                  <button 
                    ...
                    on:click={() => setActiveResource(data.resources[currentIndex + 1].id)}
                  >
                    Next: {data.resources[currentIndex + 1].title} →
                  </button>
                {/if}
              </div>
            {/if}
          {:else}
            <div class="no-resource">
              <h3>Select a Learning Resource</h3>
              <p>Choose a resource from the sidebar to view its content and guidance.</p>
            </div>
          {/if}
        </div>
      </main>
...
<style>
  /* Learning Journey color scheme - educational/growth theme */
  :root {
    ...
    --community-green: #228B22;
    --development-purple: #9932CC;
    --preservation-brown: #8B4513;
  }
