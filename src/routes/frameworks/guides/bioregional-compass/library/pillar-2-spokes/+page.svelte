<!-- src/routes/frameworks/guides/bioregional-compass/library/pillar-2-spokes/+page.svelte -->
<script>
  ...
  // Component state
  let mounted = false;
  let activeResource = 'bioregional-mapping-for-beginners'; // Start with mapping
  let activeSection = null; // For multi-section resources
  let expandedOverviews = new Set(['bioregional-mapping-for-beginners']); // Default expand first
  ...
  function getCategoryColor(category) {
    switch (category) {
      case 'mapping': return '#8B4513';
      case 'assessment': return '#228B22';
...
<svelte:head>
  <title>{getTextWithFallback('guidesBioregionalCompassLibrary.pillar2Spokes.title', 'Pillar 2: Finding Our Place - Bioregional Compass')}</title>
  <meta name="description" content="{getTextWithFallback('guidesBioregionalCompassLibrary.pillar2Spokes.description', 'Bioregional mapping, ecological assessment, and place-based understanding tools for BAZ formation')}" />
</svelte:head>
...
{#if mounted}
  ...
  <div class="breadcrumb-nav">
    <nav class="breadcrumb-container">
      ...
      <span class="breadcrumb-separator">›</span>
      <span class="breadcrumb-current">Pillar 2: Finding Our Place</span>
    </nav>
  </div>
  ...
  <div class="page-container">
    ...
    <header class="page-header">
      <div class="pillar-badge">
        <span class="pillar-number">Pillar 2</span>
      </div>
      <h1 class="page-title">
      ...
      <p class="page-subtitle">
        {bc.pillars?.pillar2?.subtitle || 'Mapping the Ecological and Cultural Contours of Your Bioregion'}
      </p>
      <p class="page-description">
        Bioregional mapping, ecological assessment, and place-based understanding tools for BAZ formation
      </p>
    </header>
    ...
    <div class="content-layout">
      ...
      <aside class="resource-nav">
        <div class="nav-header">
          <h2 class="nav-title">Bioregional Mapping Resources</h2>
          <p class="nav-subtitle">{data.totalResources} assessment toolkits</p>
        </div>
                    ...
                    {#each resource.sections as section}
                      ...
                      >
                        <span class="subsection-icon">
                          {#if section === 'overview'}📖
                          {:else if section.includes('gis') || section.includes('digital')}💻
                          {:else if section.includes('indigenous')}🌿
                          {:else if section.includes('citizen')}👥
                          {:else if section.includes('biodiversity')}🦋
                          {:else if section.includes('soil')}🌱
                          {:else if section.includes('watershed')}💧
                          {:else}🔬{/if}
                        </span>
        ...
        <div class="resource-content">
          ...
          {:else}
            <div class="no-resource">
              <h3>Select a Bioregional Mapping Resource</h3>
              <p>Choose a resource from the sidebar to view its content and assessment tools.</p>
            </div>
          {/if}
        </div>
      </main>
...
{:else}
  ...
  <div class="loading-container">
    ...
    <p>Loading bioregional mapping resources...</p>
  </div>
{/if}

<style>
  /* Pillar 2 color scheme - mapping/ecological assessment theme */
  :root {
    ...
    /* Category colors */
    --mapping-brown: #8B4513;
    --assessment-green: #228B22;
  ...
  .page-header {
    text-align: center;
    margin-bottom: 3rem;
    background: linear-gradient(135deg, var(--guide-light), rgba(34, 139, 34, 0.1));
    border-radius: 1rem;
    padding: 3rem 2rem;
  ...
  .resource-item.active {
    ...
    box-shadow: 0 0 0 1px var(--pillar-2);
  }
  ...
  .markdown-content :global(h1) {
    ...
    margin: 0 0 1.5rem 0;
    line-height: 1.2;
    border-bottom: 3px solid var(--pillar-2);
  ...
  .markdown-content :global(hr) {
    border: none;
    height: 2px;
    background: linear-gradient(to right, transparent, var(--pillar-2), transparent);
    margin: 2rem 0;
  }
  ...
  .markdown-content :global(.fieldwork) {
    margin: 1.5rem 0;
  ...
  .markdown-content :global(.fieldwork) {
    background: rgba(34, 139, 34, 0.05);
    border-left-color: var(--pillar-2);
  }
