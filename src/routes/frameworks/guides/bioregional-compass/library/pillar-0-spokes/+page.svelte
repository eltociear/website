<!-- src/routes/frameworks/guides/bioregional-compass/library/pillar-0-spokes/+page.svelte -->
<script>
  ...
  // Component state
  let mounted = false;
  let activeResource = 'inner-work-toolkit'; // Start with inner work
  let activeSection = null; // For multi-section resources
  let expandedOverviews = new Set(['inner-work-toolkit']); // Default expand first
  ...
  function getCategoryColor(category) {
    switch (category) {
      case 'personal': return '#9932CC';
      case 'leadership': return '#228B22';
      case 'healing': return '#CD853F';
      default: return '#6b7280';
    }
  }
...
<svelte:head>
  <title>{getTextWithFallback('guidesBioregionalCompassLibrary.pillar0Spokes.title', 'Pillar 0: The Inner Ground - Bioregional Compass')}</title>
  <meta name="description" content="{getTextWithFallback('guidesBioregionalCompassLibrary.pillar0Spokes.description', 'Inner work, leadership development, and consciousness cultivation for BAZ stewards')}" />
</svelte:head>
...
{#if mounted}
  ...
  <div class="breadcrumb-nav">
    <nav class="breadcrumb-container">
      ...
      <span class="breadcrumb-separator">›</span>
      <span class="breadcrumb-current">Pillar 0: The Inner Ground</span>
    </nav>
  </div>
  ...
  <div class="page-container">
    ...
    <header class="page-header">
      <div class="pillar-badge">
        <span class="pillar-number">Pillar 0</span>
      </div>
      <h1 class="page-title">
      ...
      <p class="page-subtitle">
        {bc.pillars?.pillar0?.subtitle || 'Cultivating the Consciousness for a New World'}
      </p>
      <p class="page-description">
        Inner work, leadership development, and consciousness cultivation for BAZ stewards
      </p>
    </header>
    ...
    <div class="content-layout">
      ...
      <aside class="resource-nav">
        <div class="nav-header">
          <h2 class="nav-title">Inner Work Resources</h2>
          <p class="nav-subtitle">{data.totalResources} development programs</p>
        </div>
                    ...
                    {#each resource.sections as section}
                      ...
                      >
                        <span class="subsection-icon">
                          {#if section === 'overview'}📖
                          {:else if section.includes('mindfulness')}🧘
                          {:else if section.includes('communication')}💬
                          {:else if section.includes('burnout')}🔋
                          {:else if section.includes('assessment')}📊
                          {:else if section.includes('mentorship')}🤝
                          {:else if section.includes('succession')}🔄
                          {:else}🌱{/if}
                        </span>
        ...
        <div class="resource-content">
          ...
          {:else}
            <div class="no-resource">
              <h3>Select an Inner Work Resource</h3>
              <p>Choose a resource from the sidebar to view its content and practices.</p>
            </div>
          {/if}
        </div>
      </main>
...
{:else}
  ...
  <div class="loading-container">
    ...
    <p>Loading inner work resources...</p>
  </div>
{/if}

<style>
  /* Pillar 0 color scheme - consciousness/inner work theme */
  :root {
    ...
    /* Category colors */
    --personal-purple: #9932CC;
    --leadership-green: #228B22;
    --healing-orange: #CD853F;
  }
  ...
  .page-header {
    text-align: center;
    margin-bottom: 3rem;
    background: linear-gradient(135deg, var(--guide-light), rgba(153, 50, 204, 0.1));
    border-radius: 1rem;
    padding: 3rem 2rem;
  ...
  .resource-item.active {
    border-color: var(--pillar-0);
    box-shadow: 0 0 0 1px var(--pillar-0);
  }
  ...
  /* Markdown Typography - Using :global() for component-rendered content */
  .markdown-content :global(h1) {
    ...
    margin: 0 0 1.5rem 0;
    line-height: 1.2;
    border-bottom: 3px solid var(--pillar-0);
  ...
  .markdown-content :global(blockquote) {
    margin: 1.5rem 0;
    padding: 1rem 1.5rem;
    border-left: 4px solid var(--pillar-0);
  ...
  .markdown-content :global(hr) {
    border: none;
    height: 2px;
    background: linear-gradient(to right, transparent, var(--pillar-0), transparent);
    margin: 2rem 0;
  }
  ...
  .markdown-content :global(.practice) {
    margin: 1.5rem 0;
  ...
  .markdown-content :global(.practice) {
    background: rgba(153, 50, 204, 0.05);
    border-left-color: var(--pillar-0);
  }
