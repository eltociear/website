<!-- src/routes/frameworks/guides/bioregional-compass/library/pillar-4-spokes/+page.svelte -->
<script>
  ...
  // Component state
  let mounted = false;
  let activeResource = 'decentralized-governance-starter-pack'; // Start with governance pack
  let activeSection = null; // For multi-section resources
  let expandedOverviews = new Set(['decentralized-governance-starter-pack']); // Default expand first
  ...
  onMount(async () => {
    ...
    if (hasSubsections && !activeSection) {
      // For governance, start with first section (no overview in this structure)
      activeSection = currentResource.sections[0];
    }
  });
  ...
  function setActiveResource(resourceId) {
    ...
    if (resource?.sections && resource.sections.length > 0) {
      activeSection = resource.sections[0]; // No overview, start with first section
    } else {
      activeSection = null;
    }
  ...
  function getCategoryColor(category) {
    switch (category) {
      ...
      case 'dynamics': return '#8B0000';
      case 'engagement': return '#9932CC';
      default: return '#6b7280';
    }
  }
...
<svelte:head>
  <title>{getTextWithFallback('guidesBioregionalCompassLibrary.pillar4Spokes.title', 'Pillar 4: Weaving the Council - Bioregional Compass')}</title>
  <meta name="description" content="{getTextWithFallback('guidesBioregionalCompassLibrary.pillar4Spokes.description', 'Governance formation, power dynamics management, and youth engagement for BAZ communities')}" />
</svelte:head>
...
{#if mounted}
  ...
  <div class="breadcrumb-nav">
    <nav class="breadcrumb-container">
      ...
      <span class="breadcrumb-separator">›</span>
      <span class="breadcrumb-current">Pillar 4: Weaving the Council</span>
    </nav>
  </div>
  ...
  <div class="page-container">
    ...
    <header class="page-header">
      <div class="pillar-badge">
        <span class="pillar-number">Pillar 4</span>
      </div>
      <h1 class="page-title">
      ...
      <p class="page-subtitle">
        {bc.pillars?.pillar4?.subtitle || 'Building Governance Systems Rooted in Wisdom and Participation'}
      </p>
      <p class="page-description">
        Governance formation, power dynamics management, and youth engagement for BAZ communities
      </p>
    </header>
    ...
    <div class="content-layout">
      ...
      <aside class="resource-nav">
        <div class="nav-header">
          <h2 class="nav-title">Governance Formation Resources</h2>
          <p class="nav-subtitle">{data.totalResources} governance toolkits</p>
        </div>
                    ...
                    {#each resource.sections as section}
                      ...
                      >
                        <span class="subsection-icon">
                          {#if section.includes('citizen') || section.includes('assembly')}🏛️
                          {:else if section.includes('conflict') || section.includes('resolution')}⚖️
                          {:else if section.includes('sociocratic') || section.includes('methods')}🔄
                          {:else}🤝{/if}
                        </span>
        ...
        <div class="resource-content">
          ...
          {:else}
            <div class="no-resource">
              <h3>Select a Governance Formation Resource</h3>
              <p>Choose a resource from the sidebar to view its content and governance guidance.</p>
            </div>
          {/if}
        </div>
      </main>
...
{:else}
  ...
  <div class="loading-container">
    ...
    <p>Loading governance formation resources...</p>
  </div>
{/if}

<style>
  /* Pillar 4 color scheme - governance/collaboration theme */
  :root {
    ...
    /* Category colors */
    --governance-blue: #4169E1;
    --dynamics-red: #8B0000;
    --engagement-purple: #9932CC;
  }
  ...
  .page-header {
    text-align: center;
    margin-bottom: 3rem;
    background: linear-gradient(135deg, var(--guide-light), rgba(65, 105, 225, 0.1));
    border-radius: 1rem;
    padding: 3rem 2rem;
  ...
  .resource-item.active {
    border-color: var(--pillar-4);
    box-shadow: 0 0 0 1px var(--pillar-4);
  }
  ...
  .markdown-content :global(h1) {
    ...
    margin: 0 0 1.5rem 0;
    line-height: 1.2;
    border-bottom: 3px solid var(--pillar-4);
  ...
  .markdown-content :global(blockquote) {
    margin: 1.5rem 0;
    padding: 1rem 1.5rem;
    border-left: 4px solid var(--pillar-4);
  ...
  .markdown-content :global(hr) {
    border: none;
    height: 2px;
    background: linear-gradient(to right, transparent, var(--pillar-4), transparent);
    margin: 2rem 0;
  }
  ...
  .markdown-content :global(.governance) {
    margin: 1.5rem 0;
  ...
  .markdown-content :global(.governance) {
    background: rgba(65, 105, 225, 0.05);
    border-left-color: var(--pillar-4);
  }
