<!-- src/routes/frameworks/guides/bioregional-compass/library/cross-cutting-spokes/+page.svelte -->
<script>
  ...
  // Component state
  let mounted = false;
  let activeResource = 'crisis-management-compendium'; // Start with crisis management
  let activeSection = null; // For multi-section resources
  let expandedOverviews = new Set(['crisis-management-compendium']); // Default expand first
  ...
  function getCategoryColor(category) {
    switch (category) {
      ...
      case 'growth': return '#228B22';
      case 'legal': return '#4169E1';
      case 'coordination': return '#9932CC';
      default: return '#6b7280';
    }
  }
...
<svelte:head>
  <title>{getTextWithFallback('guidesBioregionalCompassLibrary.crossCuttingSpokes.title', 'Cross-Cutting Applications - Bioregional Compass')}</title>
  <meta name="description" content="{getTextWithFallback('guidesBioregionalCompassLibrary.crossCuttingSpokes.description', 'Advanced tools for crisis management, scale transitions, legal challenges, and network coordination')}" />
</svelte:head>
...
{#if mounted}
  ...
  <div class="breadcrumb-nav">
    <nav class="breadcrumb-container">
      ...
      <span class="breadcrumb-separator">›</span>
      <span class="breadcrumb-current">Cross-Cutting Applications</span>
    </nav>
  </div>
  ...
  <div class="page-container">
    ...
    <header class="page-header">
      ...
      <p class="page-subtitle">
        Advanced tools for crisis management, scale transitions, legal challenges, and network coordination
      </p>
    </header>
    ...
    <div class="content-layout">
      ...
      <aside class="resource-nav">
        <div class="nav-header">
          ...
          <p class="nav-subtitle">{data.totalResources} specialized tools</p>
        </div>
                    ...
                    {#each resource.sections as section}
                      ...
                      >
                        <span class="subsection-icon">
                          {#if section === 'overview'}📖
                          {:else if section.includes('economic')}💰
                          {:else if section.includes('environmental') || section.includes('climate')}🌍
                          {:else if section.includes('governance') || section.includes('internal')}🏛️
                          {:else if section.includes('hostility') || section.includes('external')}🛡️
                          {:else if section.includes('legal')}⚖️
                          {:else if section.includes('mental-health')}🧠
                          {:else if section.includes('violence') || section.includes('safety')}🚨
                          {:else if section.includes('system-failure')}🔧
                          {:else if section.includes('homestead')}🏡
                          {:else if section.includes('village')}🏘️
                          {:else if section.includes('network')}🌐
                          {:else if section.includes('emergency')}⚡
                          {:else if section.includes('hostile')}⚠️
                          {:else}📄{/if}
                        </span>
...
{:else}
  ...
  <div class="loading-container">
    ...
    <p>Loading cross-cutting resources...</p>
  </div>
{/if}

<style>
  /* Cross-Cutting Spokes color scheme - advanced/specialized theme */
  :root {
