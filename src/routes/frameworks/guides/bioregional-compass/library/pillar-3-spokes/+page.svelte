<!-- src/routes/frameworks/guides/bioregional-compass/library/pillar-3-spokes/+page.svelte -->
<script>
  ...
  // Component state
  let mounted = false;
  let activeResource = 'baz-legal-primer'; // Start with legal primer
  let activeSection = null; // For multi-section resources
  let expandedOverviews = new Set(['baz-legal-primer']); // Default expand first
  ...
  function getCategoryColor(category) {
    switch (category) {
      ...
      case 'regulatory': return '#DAA520';
      case 'relations': return '#4169E1';
      default: return '#6b7280';
    }
  }
...
<svelte:head>
  <title>{getTextWithFallback('guidesBioregionalCompassLibrary.pillar3Spokes.title', 'Pillar 3: Building the Vessel - Bioregional Compass')}</title>
  <meta name="description" content="{getTextWithFallback('guidesBioregionalCompassLibrary.pillar3Spokes.description', 'Legal frameworks, regulatory navigation, and institutional scaffolding for BAZ formation')}" />
</svelte:head>
...
{#if mounted}
  ...
  <div class="breadcrumb-nav">
    <nav class="breadcrumb-container">
      ...
      <span class="breadcrumb-separator">›</span>
      <span class="breadcrumb-current">Pillar 3: Building the Vessel</span>
    </nav>
  </div>
  ...
  <div class="page-container">
    ...
    <header class="page-header">
      <div class="pillar-badge">
        <span class="pillar-number">Pillar 3</span>
      </div>
      <h1 class="page-title">
      ...
      <p class="page-subtitle">
        {bc.pillars?.pillar3?.subtitle || 'Creating Legal & Institutional Scaffolding for BAZ Aspirations'}
      </p>
      <p class="page-description">
        Legal frameworks, regulatory navigation, and institutional scaffolding for BAZ formation
      </p>
    </header>
    ...
    <div class="content-layout">
      ...
      <aside class="resource-nav">
        <div class="nav-header">
          <h2 class="nav-title">Legal & Institutional Resources</h2>
          <p class="nav-subtitle">{data.totalResources} legal toolkits</p>
        </div>
                    ...
                    {#each resource.sections as section}
                      ...
                      >
                        <span class="subsection-icon">
                          {#if section === 'overview'}📖
                          {:else if section.includes('charter') || section.includes('founding')}📜
                          {:else if section.includes('compliance') || section.includes('risk')}🛡️
                          {:else if section.includes('decision') || section.includes('structure')}🏛️
                          {:else if section.includes('building') || section.includes('construction')}🏗️
                          {:else if section.includes('tax')}💰
                          {:else if section.includes('zoning') || section.includes('land')}🗺️
                          {:else if section.includes('emergency')}🚨
                          {:else if section.includes('municipal')}🏛️
        ...
        <div class="resource-content">
          ...
          {:else}
            <div class="no-resource">
              <h3>Select a Legal & Institutional Resource</h3>
              <p>Choose a resource from the sidebar to view its content and legal guidance.</p>
            </div>
          {/if}
        </div>
      </main>
...
{:else}
  ...
  <div class="loading-container">
    ...
    <p>Loading legal & institutional resources...</p>
  </div>
{/if}

<style>
  /* Pillar 3 color scheme - legal/institutional theme */
  :root {
    ...
    /* Category colors */
    --legal-red: #8B0000;
    --regulatory-gold: #DAA520;
    --relations-blue: #4169E1;
  }
  ...
  .page-header {
    text-align: center;
    margin-bottom: 3rem;
    background: linear-gradient(135deg, var(--guide-light), rgba(139, 0, 0, 0.1));
    border-radius: 1rem;
    padding: 3rem 2rem;
  ...
  .resource-item.active {
    border-color: var(--pillar-3);
    box-shadow: 0 0 0 1px var(--pillar-3);
  }
  ...
  .markdown-content :global(h1) {
    ...
    margin: 0 0 1.5rem 0;
    line-height: 1.2;
    border-bottom: 3px solid var(--pillar-3);
  ...
  .markdown-content :global(blockquote) {
    margin: 1.5rem 0;
    padding: 1rem 1.5rem;
    border-left: 4px solid var(--pillar-3);
  ...
  .markdown-content :global(hr) {
    border: none;
    height: 2px;
    background: linear-gradient(to right, transparent, var(--pillar-3), transparent);
    margin: 2rem 0;
  }
  ...
  .markdown-content :global(.legal) {
    background: rgba(139, 0, 0, 0.05);
    border-left-color: var(--pillar-3);
  }
