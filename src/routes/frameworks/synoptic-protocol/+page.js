// src/routes/frameworks/synoptic-protocol/+page.js
...
export async function load({ depends, url, params }) {
  ...
  if (DEBUG_FRAMEWORK_LOADING) console.log('=== Synoptic Protocol +page.js load function ===');
  ...
  try {
    // The pathname should always be /frameworks/synoptic-protocol
    ...
    if (cleanPath.includes('/frameworks/') && 
        (cleanPath.includes('introduction') ||
         cleanPath.includes('epistemic-crisis') ||
         cleanPath.includes('foundational-principles') ||
         cleanPath.includes('six-pillars') ||
         cleanPath.includes('implementation-governance') ||
      ...
      cleanPath = '/frameworks/synoptic-protocol';
    }
  ...
  // Define sections to load - synoptic protocol framework sections in correct order
  const sections = [
  ...
  if (DEBUG_FRAMEWORK_LOADING) console.log('Loading synoptic protocol sections for locale:', currentLocale);
  
  // Try to load each section with proper error handling
  for (const section of sections) {
    try {
      // Try to load the current locale version first
      const modulePromise = import(`$lib/content/frameworks/${currentLocale}/implementation/synoptic-protocol/${section}.md`);
      ...
      try {
        const fallbackPromise = import(`$lib/content/frameworks/en/implementation/synoptic-protocol/${section}.md`);
  ...
  if (!content.index) {
    ...
    throw error(500, {
      message: 'Failed to load synoptic protocol content',
  ...
  return {
    ...
    // Additional metadata for synoptic protocol framework
    frameworkType: 'synoptic-protocol',
    totalSections: sections.length,
    coreFrameworkSections: 8, // introduction through advocacy-action
    foundationSections: 2, // at-a-glance and executive-summary
    resourceSections: 2, // glossary and appendices
    hasExecutiveSummary: true,
    
    // Synoptic Protocol-specific metadata
    protocolVersion: '3.1',
    tier: 2,
    epistemic: {
      threatTiers: 6, // Tier 0 through Tier -1, plus cross-tier driver
      pillars: 6, // The six pillars of epistemic integrity
      principles: 10, // Ten foundational principles
      rightToReality: true
    },
    implementation: {
      phases: 3, // Three-phase implementation roadmap
      tierAdoptionLevels: 3, // Tiered adoption pathways
      pilotRegions: 10,
      targetInstitutions: 100
    },
    isMediaFramework: true,
    isEpistemicFramework: true,
    isDemocracyFramework: true,
    ...
    debug: {
      ...
      pathHandling: {
        ...
        cleanedPath: '/frameworks/synoptic-protocol'
      },
