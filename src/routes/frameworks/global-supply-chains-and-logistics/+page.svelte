<!-- src/routes/frameworks/global-supply-chains-and-logistics/+page.svelte -->
<script>
  ...
  // Extract GSCL translations for shorter references
  $: gscl = $t('gscl') || {};
  ...
  // Define section groupings - match the GSCL structure
  const sectionGroups = {
    overview: ['preamble', 'index', 'at-a-glance', 'executive-summary-for-the-skeptic'],
    foundations: ['introduction', 'theory-of-change', 'framework-position'],
    implementation: ['core-pillars', 'economic-integration', 'technology-infrastructure'],
    governance: ['governance-compliance', 'implementation-roadmap', 'monitoring-evaluaton'],
    integration: ['global-alignment', 'appendices']
  };
  ...
  function getSectionTitle(section) {
    return gscl.sections?.[section] || section;
  }

  // Get section category titles using short references
  function getSectionCategoryTitle(category) {
    return gscl.categories?.[category] || category;
  }

  // Function to get shortened section titles for navigation using short references
  function getShortSectionTitle(section) {
    return gscl.sectionsShort?.[section] || getSectionTitle(section);
  }

  // Function to download the GSCL guide PDF
  function downloadGSCLGuide(version = '') {
    const versionSuffix = version ? `-${version}` : '';
    const pdfUrl = `${base}/assets/pdf/gscl-framework-guide${versionSuffix}-${currentLocale}.pdf`;
    const link = document.createElement('a');
    link.href = pdfUrl;
    link.download = `gscl-framework-guide${versionSuffix}.pdf`;
  ...
  $: quickStartGuides = gscl.quickStart?.guides || [
    {
      id: 'at-a-glance',
      icon: '⚡',
      title: 'At a Glance',
      description: '30-second overview of regenerative supply chains'
    },
    {
      id: 'executive-summary-for-the-skeptic',
      icon: '🤔',
      title: 'For Skeptics',
      description: 'Evidence-based case for transformation'
    }
  ];
...
{#if mounted}
  <div class="documentation-container">
    ...
    <div class="content">
      <!-- Quick Access Card for GSCL Quick Start -->
      {#if !isPrintMode && !isQuickStartActive && (activeSection === 'index' || activeSection === 'preamble')}
        <div class="gscl-guide-card">
          <div class="card-content">
            <div class="card-icon">🚚</div>
            <div class="card-text">
              <h3>{gscl.quickStart?.new || 'New to Supply Chain Transformation?'}</h3>
              <p>{gscl.quickStart?.start || 'Start with our quick guides that explain regenerative supply chains for different audiences.'}</p>
            </div>
            <div class="card-actions">
              <div class="dropdown">
                <button 
                  ...
                  type="button"
                >
                  {gscl.quickStart?.button || 'Choose Guide'} 
      ...
      <!-- Sub-navigation for GSCL framework sections -->
      {#if !isPrintMode && !initializing} 
        <div class="section-nav">
          ...
          <!-- Implementation & Core Systems Accordion -->
          <div class="nav-accordion">
            <button 
              class="accordion-header" 
          ...
          <!-- Governance & Evaluation Accordion -->
          <div class="nav-accordion">
            <button 
              class="accordion-header" 
          ...
          <!-- Integration & Resources Accordion -->
          <div class="nav-accordion">
            <button 
              class="accordion-header" 
      ...
      {#if !isPrintMode && isCoreSection && allCoreSections.length > 0}
        <div class="progress-indicator">
          ...
          <span class="progress-text">{gscl.progress?.text?.replace('{current}', allCoreSections.indexOf(activeSection) + 1).replace('{total}', allCoreSections.length) || `Section ${allCoreSections.indexOf(activeSection) + 1} of ${allCoreSections.length}`}</span>
        </div>
      {/if}
      ...
      {#each sectionsToShow as section (section)}
            ...
            {#if !isPrintMode && data.sectionsUsingEnglishFallback?.includes(section)}
              <div class="language-fallback-notice">
                ...
                <div class="notice-content">
                  <strong>{gscl.languageFallback?.title || 'Content in your language coming soon'}</strong>
                  <p>{gscl.languageFallback?.description || 'This section is currently displayed in English until translation is complete.'}</p>
                </div>
              </div>
            {/if}
            ...
            {#if allQuickStartSections.includes(section) && section !== 'index' && section !== 'preamble'}
              ...
              {#if !isPrintMode}
                <div class="guide-selector">
                  <h2>{gscl.quickStart?.title || 'GSCL Framework Quick Start'}</h2>
                  <p>{gscl.quickStart?.description || 'Choose the version that best matches your needs:'}</p>
              ...
              {#if !isPrintMode}
                <div class="guide-navigation">
                  <button class="secondary-btn" on:click={() => downloadGSCLGuide(section.replace('-', ''))}>
                    {gscl.quickStart?.download || 'Download PDF Version'} <span class="download-icon">↓</span>
                  </button>
                  <button class="primary-btn" on:click={() => setActiveSection('introduction')}>
                    {gscl.quickStart?.continue || 'Continue to Full Framework'} <span class="arrow-icon">→</span>
                  </button>
                </div>
              {/if}
            {:else}
            ...
            {#if activeSection === 'preamble' && !isPrintMode}
              <div class="preamble-navigation">
                <button class="primary-btn" on:click={() => setActiveSection('index')}>
                  {gscl.preamble?.continue || 'Continue to Framework Overview'} 
              ...
              <div class="section-navigation">
                {#if allCoreSections.indexOf(activeSection) > 0}
                  <button class="nav-btn prev-btn" on:click={() => {
                    ...
                    ← {gscl.navigation?.previousSection || 'Previous Section'}
                  </button>
                {/if}
                ...
                {#if allCoreSections.indexOf(activeSection) < allCoreSections.length - 1}
                  <button class="nav-btn next-btn" on:click={() => {
                    ...
                    {gscl.navigation?.nextSection || 'Next Section'} →
                  </button>
                {/if}
              </div>
            {/if}
        ...
        {:else}
          <div class="missing-section">
            <h2>{gscl.errors?.sectionNotFound?.replace('{section}', section) || `Section "${section}" not found`}</h2>
            <p>{gscl.errors?.contentInDevelopment || 'This content is still being developed.'}</p>
          </div>
        {/if}
      {/each}
    </div>
...
{:else}
  ...
  <div class="loading-container">
    ...
    <p>{gscl.loading?.text || 'Loading supply chain framework content...'}</p>
  </div>
{/if}

<style>
  /* GSCL Framework color scheme - Supply Chain & Logistics themed */
  :root {
    --gscl-primary: #2E7D32; /* Forest green for regenerative nature */
    --gscl-secondary: #4CAF50; /* Brighter green for growth */
    --gscl-accent: #FF9800; /* Orange for energy/movement */
    --gscl-success: #1B5E20; /* Dark green for stability */
    --gscl-warning: #F57C00; /* Amber for attention */
    --gscl-danger: #C62828; /* Red for alerts */
    --gscl-light: #E8F5E8; /* Very light green */
    --gscl-dark: #1B5E20; /* Dark forest green */
    --gscl-hearts: #E91E63; /* Pink-red for Hearts currency */
    --gscl-leaves: #4CAF50; /* Green for Leaves currency */
  }
  ...
  .nav-item.active {
    background-color: var(--gscl-primary);
    color: white;
    font-weight: 600;
  }

  .nav-item.active:hover {
    background-color: var(--gscl-secondary);
  }
  ...
  .featured-item.active {
    background: var(--gscl-primary);
    border-color: var(--gscl-primary);
  }
  ...
  .primary-btn:hover {
    background-color: var(--gscl-dark);
  ...
  .secondary-btn {
    ...
    color: var(--gscl-primary);
    border: 1px solid var(--gscl-primary);
    padding: 0.5rem 1rem;
    border-radius: 0.375rem;
    font-weight: 500;
    cursor: pointer;
  ...
  .secondary-btn:hover {
    background-color: var(--gscl-light);
    transform: translateY(-1px);
  }
  ...
  .progress-indicator {
    ...
    border-radius: 0.5rem;
    border-left: 4px solid var(--gscl-primary);
  }
  ...
  .progress-fill {
    height: 100%;
    background: linear-gradient(90deg, var(--gscl-primary), var(--gscl-secondary));
    border-radius: 4px;
  ...
  :global(.content thead) {
    background: linear-gradient(to right, var(--gscl-primary), var(--gscl-secondary));
  }
  ...
  :global(.content th) {
    ...
    color: #ffffff;
    border: none;
    border-bottom: 2px solid var(--gscl-primary);
  }
  ...
  :global(.content table tfoot) {
    ...
    color: var(--gscl-primary);
    text-align: left;
    border-top: 1px solid var(--gscl-primary);
  }
  ...
  .loading-spinner {
    width: 3rem;
    height: 3rem;
    border: 3px solid var(--gscl-light);
    border-top: 3px solid var(--gscl-primary);
    border-radius: 50%;
  ...
  .nav-item[data-section="preamble"] {
    ...
    border-left: 3px solid var(--gscl-primary);
  }
</style>
