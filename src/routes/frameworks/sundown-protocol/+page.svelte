<!-- src/routes/frameworks/sundown-protocol/+page.svelte -->
<script>
  ...
  $: sf = translationsReady ? ($t('sundownProtocol') || {}) : {};
  ...
  $: coreFrameworkSections = Object.keys(data?.sections || {}).filter(section => 
    !['index', 'at-a-glance', 'executive-summary-for-the-skeptic', 'faq', 'taking-action', 'appendices'].includes(section)
  );
  ...
  onMount(async () => {
    ...
    if (browser) {
      ...
      if (window.location.pathname !== '/frameworks/sundown-protocol') {
        ...
        // Correct the URL
        const correctUrl = `/frameworks/sundown-protocol${window.location.search}${extractedHash}`;
      ...
      if (!translationsReady) {
        try {
          await loadTranslations($locale, '/frameworks/sundown-protocol');
        } catch (e) {
  ...
  function setActiveSection(section) {
    ...
    if (browser) {
      const newUrl = `/frameworks/sundown-protocol${window.location.search}#${section}`;
  ...
  function downloadProtocol(version = '') {
    ...
    const pdfUrl = `${base}/assets/pdf/sundown-protocol${versionSuffix}-${currentLocale}.pdf`;
    const link = document.createElement('a');
    link.href = pdfUrl;
    link.download = `sundown-protocol${versionSuffix}.pdf`;
...
<svelte:head>
  <title>{getTextWithFallback('sundownProtocol.meta.title', 'The Sundown Protocol - Just Transition Framework')}</title>
  <meta name="description" content="{getTextWithFallback('sundownProtocol.meta.description', 'A comprehensive framework for retiring harmful industries and enabling just transition to regenerative futures')}" />
</svelte:head>
...
{#if mounted}
  <div class="documentation-container">
    ...
    <div class="content">
      <!-- Quick Access Card for Sundown Protocol -->
      {#if !isPrintMode && !isExecutiveSummaryActive && activeSection === 'index' && translationsReady}
        <div class="protocol-guide-card">
          <div class="card-content">
            <div class="card-icon">🌇</div>
            <div class="card-text">
              <h3>{sf.guideCard?.title || 'New to the Sundown Protocol?'}</h3>
              <p>{sf.guideCard?.description || 'Start with our executive summary designed specifically for skeptics—addressing practical concerns about just transition and industry transformation.'}</p>
            </div>
                  ...
                  {#each coreFrameworkSections as section}
                    ...
                    >
                      <span class="nav-icon">
                        ...
                        {:else if section === 'historical-grounding'}📜
                        {:else if section === 'strategic-resilience'}🛡️
                        {:else if section === 'economic-modeling'}📊
                        {:else if section === 'stakeholder-integration'}🤝
                        {:else if section === 'implementation-phases'}📋
                        {:else if section === 'core-components'}⚙️
                        {:else if section === 'justice-reparations'}⚖️
      ...
      {#each sectionsToShow as section (section)}
        ...
        {:else}
          <div class="missing-section">
            <h2>{getTextWithFallback('sundownProtocol.errors.sectionNotFound', `Section "${section}" not found`).replace('{section}', section)}</h2>
            <p>{getTextWithFallback('sundownProtocol.errors.contentInDevelopment', 'This content is still being developed.')}</p>
          </div>
        {/if}
      {/each}
    </div>
...
{:else}
  ...
  <div class="loading-container">
    ...
    <p>{getTextWithFallback('sundownProtocol.loading.text', 'Loading protocol content...')}</p>
  </div>
{/if}

<style>
  /* Sundown Protocol color scheme - Sunset/Twilight inspired */
  :root {
    --sundown-primary: #8B4513;     /* Saddle brown - earth/industry */
    --sundown-secondary: #FF6B35;   /* Sunset orange - transformation */
    --sundown-accent: #C44536;      /* Rust red - transitions */
    --sundown-success: #228B22;     /* Forest green - regeneration */
    --sundown-warning: #DAA520;     /* Goldenrod - caution/planning */
    --sundown-danger: #DC143C;      /* Crimson - urgency */
    --sundown-light: #FFF8DC;       /* Cornsilk - warmth */
    --sundown-dark: #2F1B14;        /* Dark brown - earth */
    --sundown-wisdom: #4682B4;      /* Steel blue - guidance */
    --sundown-twilight: #663399;    /* Purple - evening transition */
  }
  ...
  .loading-spinner {
    ...
    border-top: 4px solid var(--sundown-primary);
    border-radius: 50%;
  ...
  .section-nav {
    ...
    background: linear-gradient(to bottom, var(--sundown-light), #f1f5f9);
    border-radius: 0.5rem;
    padding: 1rem;
  }
  ...
  .accordion-header:focus-visible {
    outline: 2px solid var(--sundown-accent);
    outline-offset: 2px;
  ...
  .nav-item.active {
    background-color: var(--sundown-primary);
    color: white;
    font-weight: 600;
  }

  .nav-item.active:hover {
    background-color: var(--sundown-secondary);
  }

  .overview-item {
    background: linear-gradient(135deg, rgba(139, 69, 19, 0.1), rgba(255, 107, 53, 0.1));
  ...
  .overview-item.active {
    background: var(--sundown-primary);
    color: white;
  }
  ...
  .progress-indicator {
    margin-bottom: 2rem;
    padding: 1rem;
    background: linear-gradient(90deg, rgba(139, 69, 19, 0.1), rgba(255, 107, 53, 0.1));
    border-radius: 0.5rem;
    border-left: 4px solid var(--sundown-primary);
  }
  ...
  .progress-fill {
    height: 100%;
    background: linear-gradient(90deg, var(--sundown-primary), var(--sundown-secondary));
    border-radius: 4px;
  ...
  .secondary-btn {
    ...
    color: var(--sundown-primary);
    border: 1px solid var(--sundown-primary);
    padding: 0.5rem 1rem;
    border-radius: 0.375rem;
    font-weight: 500;
    cursor: pointer;
  ...
  .content :global(blockquote) {
    background-color: rgba(255, 107, 53, 0.1);
    border-left: 4px solid var(--sundown-secondary);
    padding: 1rem 1.5rem;
    margin: 1.5rem 0;
    border-radius: 0.5rem;
  }

  /* Lists - using regular bullets as requested */
