// src/routes/frameworks/sundown-protocol/+page.js
...
export async function load({ depends, url, params }) {
  ...
  try {
    // The pathname should always be /frameworks/sundown-protocol
    ...
    if (cleanPath.includes('/frameworks/') && 
        (cleanPath.includes('executive-summary') || 
         ...
         cleanPath.includes('stakeholder-integration') ||
         cleanPath.includes('implementation-phases') ||
         cleanPath.includes('core-components') ||
         cleanPath.includes('justice-reparations') ||
      ...
      cleanPath = '/frameworks/sundown-protocol';
    }
  ...
  // Define sections to load - sundown protocol sections in correct order
  const sections = [
    ...
    // Action and supplementary materials
    'faq',
    'taking-action',
    'appendices'
  ];
  ...
  if (DEBUG_FRAMEWORK_LOADING) console.log('Loading sundown protocol sections for locale:', currentLocale);
  
  // Try to load each section with proper error handling
  for (const section of sections) {
    try {
      // Try to load the current locale version first
      const modulePromise = import(`$lib/content/frameworks/${currentLocale}/implementation/sundown-protocol/${section}.md`);
      ...
      try {
        const fallbackPromise = import(`$lib/content/frameworks/en/implementation/sundown-protocol/${section}.md`);
  ...
  if (!content.index) {
    ...
    throw error(500, {
      message: 'Failed to load sundown protocol content',
  ...
  return {
    ...
    // Additional metadata for sundown protocol framework
    frameworkType: 'sundown-protocol',
    totalSections: sections.length,
    coreFrameworkSections: 11, // introduction-vision through success-metrics
    foundationSections: 2, // at-a-glance and executive-summary
    resourceSections: 3, // faq, taking-action, appendices
    hasExecutiveSummary: true,
    
    // Sundown Protocol-specific metadata
    protocolVersion: '1.0',
    isJustTransitionFramework: true,
    implementationPhases: 5, // Pre-Mandate through Phase 4
    tier: 2, // Foundational Application Framework
    targetIndustries: ['fossil-fuels', 'industrial-agriculture', 'extractive-industries'],
    ...
    debug: {
      ...
      pathHandling: {
        ...
        cleanedPath: '/frameworks/sundown-protocol'
      },
