// src/routes/frameworks/cultural-heritage-preservation/+page.js
...
export async function load({ depends, url, params }) {
  ...
  try {
    ...
    if (cleanPath.includes('/frameworks/') && 
      ...
      cleanPath = '/frameworks/cultural-heritage-preservation';
    }
  ...
  // Define sections to load - cultural heritage framework sections in correct order
  const sections = [
  ...
  if (DEBUG_FRAMEWORK_LOADING) console.log('Loading cultural heritage sections for locale:', currentLocale);
  
  // Try to load each section with proper error handling
  for (const section of sections) {
    try {
      // Try to load the current locale version first
      const modulePromise = import(`$lib/content/frameworks/${currentLocale}/implementation/cultural-heritage-preservation/${section}.md`);
      ...
      try {
        const fallbackPromise = import(`$lib/content/frameworks/en/implementation/cultural-heritage-preservation/${section}.md`);
  ...
  if (!content.index) {
    ...
    throw error(500, {
      message: 'Failed to load cultural heritage content',
  ...
  return {
    ...
    // Additional metadata for cultural heritage framework
    frameworkType: 'cultural-heritage-preservation',
    totalSections: sections.length,
    coreFrameworkSections: 10, // introduction-vision through taking-action
    foundationSections: 2, // at-a-glance and executive-summary
    resourceSections: 1, // appendices
    hasExecutiveSummary: true,
    
    // Cultural heritage-specific metadata
    frameworkVersion: '2.2',
    tier: 3,
    category: 'Cultural Memory Operating System',
    ...
    debug: {
      ...
      pathHandling: {
        ...
        cleanedPath: '/frameworks/cultural-heritage-preservation'
      },
