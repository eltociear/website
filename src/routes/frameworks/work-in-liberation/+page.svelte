<!-- src/routes/frameworks/work-in-liberation/+page.svelte -->
<script>
 ...
 // Extract Work in Liberation translations for shorter references
 ...
 // Section icons - tailored to work liberation and cooperative organizing theme
 const sectionIcons = {
   // Overview cluster
   'index': '👥',
   'at-a-glance': '⚡',
 ...
 function initializeAccordionStates() {
   ...
   } else if (sectionGroups.framework.includes(activeSection)) {
     frameworkOpen = true;
   } else if (sectionGroups.implementation.includes(activeSection)) {
     implementationOpen = true;
   } else if (sectionGroups.integration.includes(activeSection)) {
 ...
 function getSectionTitle(section) {
   return workInLiberation.sections?.[section] || section;
 }

 // Get section category titles using short references
 function getSectionCategoryTitle(category) {
   return workInLiberation.categories?.[category] || category;
 }

 // Function to get shortened section titles for navigation using short references
 function getShortSectionTitle(section) {
   return workInLiberation.sectionsShort?.[section] || getSectionTitle(section);
 }

 // Function to download the Work in Liberation guide PDF
 function downloadWorkInLiberationGuide(version = '') {
   const versionSuffix = version ? `-${version}` : '';
   const pdfUrl = `${base}/assets/pdf/work-in-liberation-framework${versionSuffix}-${currentLocale}.pdf`;
   const link = document.createElement('a');
   link.href = pdfUrl;
   link.download = `work-in-liberation-framework${versionSuffix}.pdf`;
 ...
 $: allCoreSections = [
   ...
   ...sectionGroups.framework,
   ...sectionGroups.implementation,
   ...sectionGroups.integration,
   ...sectionGroups.resources
 ];
 $: isCoreSection = allCoreSections.includes(activeSection);

 // Get quick start guides from translations
 $: quickStartGuides = workInLiberation.quickStart?.guides || [
   {
     id: 'at-a-glance',
     icon: '⚡',
     title: 'At a Glance',
     description: '5-minute overview of transformative work organization'
   },
   {
     id: 'executive-summary-for-the-skeptic',
     icon: '🤔',
     title: 'For Skeptics',
     description: 'Business case for democratic work transformation'
   }
 ];
...
{#if mounted}
 <div class="documentation-container">
   ...
   <div class="content">
     <!-- Quick Access Card for Work in Liberation Quick Start -->
     {#if !isPrintMode && !isQuickStartActive && (activeSection === 'index' || activeSection === 'introduction')}
       <div class="work-liberation-guide-card">
         <div class="card-content">
           <div class="card-icon">👥</div>
           <div class="card-text">
             <h3>{workInLiberation.quickStart?.new || 'New to Work in Liberation?'}</h3>
             <p>{workInLiberation.quickStart?.start || 'Start with our quick guides that explain transforming work from exploitation to meaningful contribution.'}</p>
           </div>
           <div class="card-actions">
             <div class="dropdown">
               <button 
                 ...
                 type="button"
               >
                 {workInLiberation.quickStart?.button || 'Choose Guide'} 
     ...
     <!-- Sub-navigation for Work in Liberation framework sections -->
       ...
       <div class="section-nav">
         ...
         <div class="nav-accordion">
           <button 
             ...
             class:has-active={sectionGroups.framework.some(section => activeSection === section)}
             on:click={toggleFramework}
           >
             <span class="accordion-icon">🏗️</span>
             <span class="accordion-title">{getSectionCategoryTitle('framework')}</span>
             <span class="section-count">({sectionGroups.framework.length})</span>
             <span class="toggle-arrow" class:rotated={frameworkOpen}>▼</span>
           </button>
           {#if frameworkOpen}
             <div class="accordion-content" transition:slide={{ duration: 200 }}>
               {#each sectionGroups.framework as section}
                 <button 
                   class="nav-item subsection-item" 
                   class:active={activeSection === section}
                   on:click={() => setActiveSection(section)}
         ...
         <!-- Implementation & Assessment Accordion -->
         <div class="nav-accordion">
           <button 
             class="accordion-header" 
         ...
         <!-- Integration & Global Strategy Accordion -->
         <div class="nav-accordion">
           <button 
             class="accordion-header" 
             class:open={integrationOpen}
             class:has-active={sectionGroups.integration.some(section => activeSection === section)}
             on:click={toggleIntegration}
           >
             <span class="accordion-icon">🔗</span>
             <span class="accordion-title">{getSectionCategoryTitle('integration')}</span>
             <span class="section-count">({sectionGroups.integration.length})</span>
             <span class="toggle-arrow" class:rotated={integrationOpen}>▼</span>
           </button>
           {#if integrationOpen}
             <div class="accordion-content" transition:slide={{ duration: 200 }}>
               {#each sectionGroups.integration as section}
                 <button 
                   class="nav-item subsection-item" 
                   class:active={activeSection === section}
                   on:click={() => setActiveSection(section)}
     ...
     {#if !isPrintMode && isCoreSection && allCoreSections.length > 0}
       <div class="progress-indicator">
         ...
         <span class="progress-text">{workInLiberation.progress?.text?.replace('{current}', allCoreSections.indexOf(activeSection) + 1).replace('{total}', allCoreSections.length) || `Section ${allCoreSections.indexOf(activeSection) + 1} of ${allCoreSections.length}`}</span>
       </div>
     {/if}
     ...
     {#each sectionsToShow as section (section)}
         ...
         <div class="section-content" id={section}>
           ...
           {#if !isPrintMode && data.sectionsUsingEnglishFallback?.includes(section)}
             <div class="language-fallback-notice">
               ...
               <div class="notice-content">
                 <strong>{workInLiberation.languageFallback?.title || 'Content in your language coming soon'}</strong>
                 <p>{workInLiberation.languageFallback?.description || 'This section is currently displayed in English until translation is complete.'}</p>
               </div>
             </div>
           {/if}
           ...
           {#if allQuickStartSections.includes(section) && section !== 'index' && section !== 'introduction'}
             ...
             {#if !isPrintMode}
               <div class="guide-selector">
                 <h2>{workInLiberation.quickStart?.title || 'Work in Liberation Quick Start'}</h2>
                 <p>{workInLiberation.quickStart?.description || 'Choose the version that best matches your needs:'}</p>
             ...
             {#if !isPrintMode}
               <div class="guide-navigation">
                 <button class="secondary-btn" on:click={() => downloadWorkInLiberationGuide(section.replace('-', ''))}>
                   {workInLiberation.quickStart?.download || 'Download PDF Version'} <span class="download-icon">↓</span>
                 </button>
                 <button class="primary-btn" on:click={() => setActiveSection('introduction')}>
                   {workInLiberation.quickStart?.continue || 'Continue to Full Framework'} <span class="arrow-icon">→</span>
                 </button>
               </div>
             {/if}
           {:else}
           ...
           {#if activeSection === 'introduction' && !isPrintMode}
             <div class="introduction-navigation">
               <button class="primary-btn" on:click={() => setActiveSection('core-principles')}>
                 {workInLiberation.introduction?.continue || 'Continue to Core Principles'} 
             ...
             <div class="section-navigation">
               {#if allCoreSections.indexOf(activeSection) > 0}
                 <button class="nav-btn prev-btn" on:click={() => {
                   ...
                   ← {workInLiberation.navigation?.previousSection || 'Previous Section'}
                 </button>
               {/if}
               ...
               {#if allCoreSections.indexOf(activeSection) < allCoreSections.length - 1}
                 <button class="nav-btn next-btn" on:click={() => {
                   ...
                   {workInLiberation.navigation?.nextSection || 'Next Section'} →
                 </button>
               {/if}
             </div>
           {/if}
       ...
       {:else}
         <div class="missing-section">
           <h2>{workInLiberation.errors?.sectionNotFound?.replace('{section}', section) || `Section "${section}" not found`}</h2>
           <p>{workInLiberation.errors?.contentInDevelopment || 'This content is still being developed.'}</p>
         </div>
       {/if}
     {/each}
   </div>
...
{:else}
 ...
 <div class="loading-container">
   ...
   <p>{workInLiberation.loading?.text || 'Loading work liberation framework...'}</p>
 </div>
{/if}

<style>
 /* Scandinavian Professional & Calm theme for Work in Liberation */
 :root {
   --primary-brand: #2C5282;     /* Deep, trustworthy blue (for headers, primary buttons) */
   --accent-action: #2F855A;     /* Vibrant, earthy green (for CTAs, highlights, success) */
   --text-primary: #1A202C;      /* Very dark charcoal (instead of pure black for text) */
   --text-secondary: #4A5568;    /* Muted gray for subtitles and secondary info */
   --background-main: #F7FAFC;   /* Clean, slightly off-white background */
   --background-alt: #EDF2F7;    /* Light gray for cards or alternate sections */
   --border-color: #E2E8F0;      /* Subtle border color */
   /* Semantic Colors */
   --color-warning: #DD6B20;     /* A warmer, less alarming orange */
   --color-danger: #C53030;      /* A deeper, more grounded red for errors */
   --color-community: #5A67D8;   /* A friendly indigo for tags or icons */
   
   /* Work in Liberation specific colors */
   --work-liberation-primary: var(--primary-brand);
   --work-liberation-secondary: #2B6CB0;  /* Slightly lighter blue */
   --work-liberation-accent: var(--accent-action);
   --work-liberation-cooperative: var(--color-community);
   --work-liberation-care: #B794F6;       /* Soft purple for care work */
   --work-liberation-tools: #4299E1;      /* Light blue for tools */
   --work-liberation-light: #EBF8FF;      /* Very light blue */
   --work-liberation-dark: #1A365D;       /* Dark blue */
 }
 ...
 .section-nav {
   ...
   background: linear-gradient(to bottom, var(--background-main), var(--background-alt));
 ...
 .accordion-header.has-active {
   ...
   color: var(--work-liberation-primary);
   font-weight: 600;
 }
 ...
 .overview-header {
   background: linear-gradient(135deg, rgba(44, 82, 130, 0.1), rgba(43, 108, 176, 0.1));
   border-bottom: 1px solid rgba(44, 82, 130, 0.2);
 }

 .overview-header.has-active {
   background: linear-gradient(135deg, rgba(44, 82, 130, 0.15), rgba(43, 108, 176, 0.15));
 }
 ...
 .nav-item.active {
   background-color: var(--work-liberation-primary);
   color: white;
   font-weight: 600;
 }

 .nav-item.active:hover {
   background-color: var(--work-liberation-secondary);
 }

 /* Special styling for featured items (at-a-glance and executive summary) */
 .featured-item {
   background: linear-gradient(135deg, rgba(44, 82, 130, 0.08), rgba(43, 108, 176, 0.08));
 ...
 .featured-item.active {
   background: var(--work-liberation-primary);
   border-color: var(--work-liberation-primary);
 }
 ...
 .featured-badge {
   ...
   background: var(--work-liberation-cooperative);
   color: white;
   font-size: 0.7rem;
   font-weight: 600;
 ...
 .work-liberation-guide-card .dropdown-menu {
   position: absolute;
   top: 100%;
   left: 0;
   z-index: 1001;
 ...
 .primary-btn:hover {
   background-color: var(--work-liberation-dark);
 ...
 .secondary-btn {
   ...
   color: var(--work-liberation-primary);
   border: 1px solid var(--work-liberation-primary);
   padding: 0.5rem 1rem;
   border-radius: 0.375rem;
   font-weight: 500;
   cursor: pointer;
 ...
 .secondary-btn:hover {
   background-color: var(--work-liberation-light);
   transform: translateY(-1px);
 }
 ...
 .progress-indicator {
   margin-bottom: 2rem;
   padding: 1rem;
   background: linear-gradient(90deg, rgba(44, 82, 130, 0.1), rgba(43, 108, 176, 0.1));
   border-radius: 0.5rem;
   border-left: 4px solid var(--work-liberation-primary);
 }
 ...
 .progress-fill {
   height: 100%;
   background: linear-gradient(90deg, var(--work-liberation-primary), var(--work-liberation-secondary));
   border-radius: 4px;
 ...
 .content :global(h2) {
   ...
   margin-top: 2rem;
   margin-bottom: 1rem;
   color: var(--work-liberation-secondary);
 }
 ...
 :global(h4) {
   ...
   color: var(--work-liberation-tools);
 }
 ...
 :global(blockquote a:hover) {
   color: var(--work-liberation-dark);
 }
 ...
 :global(.content thead) {
   background: linear-gradient(to right, var(--work-liberation-primary), var(--work-liberation-secondary));
 }
 ...
 :global(.content th) {
   ...
   color: #ffffff;
   border: none;
   border-bottom: 2px solid var(--work-liberation-primary);
 }
 ...
 :global(.content table tfoot) {
   ...
   color: var(--work-liberation-primary);
   text-align: left;
   border-top: 1px solid var(--work-liberation-primary);
 }
 ...
 .loading-spinner {
   width: 3rem;
   height: 3rem;
   border: 3px solid var(--work-liberation-light);
   border-top: 3px solid var(--work-liberation-primary);
   border-radius: 50%;
 ...
 .nav-item[data-section="introduction"] {
   font-weight: 600;
   background: linear-gradient(135deg, rgba(44, 82, 130, 0.05), rgba(43, 108, 176, 0.05));
   border-left: 3px solid var(--work-liberation-primary);
 }
</style>
