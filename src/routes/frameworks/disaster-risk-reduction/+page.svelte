...
<script>
  ...
  $: tf = translationsReady ? ($t('drrFramework') || {}) : {};
  ...
  onMount(async () => {
    ...
    if (browser) {
      ...
      if (window.location.pathname !== '/frameworks/disaster-risk-reduction') {
        ...
        // Correct the URL
        const correctUrl = `/frameworks/disaster-risk-reduction${window.location.search}${extractedHash}`;
      ...
      if (!translationsReady) {
        try {
          await loadTranslations($locale, '/frameworks/disaster-risk-reduction');
        } catch (e) {
  ...
  function setActiveSection(section) {
    ...
    if (browser) {
      const newUrl = `/frameworks/disaster-risk-reduction${window.location.search}#${section}`;
  ...
  // Function to download the DRR framework PDF
  function downloadFramework(version = '') {
    const versionSuffix = version ? `-${version}` : '';
    const pdfUrl = `${base}/assets/pdf/disaster-risk-reduction${versionSuffix}-${currentLocale}.pdf`;
    const link = document.createElement('a');
    link.href = pdfUrl;
    link.download = `disaster-risk-reduction${versionSuffix}.pdf`;
...
<svelte:head>
  <title>{getTextWithFallback('drrFramework.meta.title', 'Disaster Risk Reduction & Resilience Framework - Building Planetary Resilience')}</title>
  <meta name="description" content="{getTextWithFallback('drrFramework.meta.description', 'A comprehensive framework for transforming disaster management into community resilience and adaptive capacity')}" />
</svelte:head>
...
{#if mounted}
  <div class="documentation-container">
    ...
    <div class="content">
      ...
      {#if !isPrintMode && !isExecutiveSummaryActive && activeSection === 'index' && translationsReady}
        <div class="drr-guide-card">
          <div class="card-content">
            ...
            <div class="card-text">
              <h3>{tf.guideCard?.title || 'New to Disaster Risk Reduction & Resilience?'}</h3>
              <p>{tf.guideCard?.description || 'Start with our executive summary designed for skeptics—addressing practical concerns about transforming disaster response into community resilience.'}</p>
            </div>
      ...
      <!-- Sub-navigation for DRR framework sections -->
      {#if !isPrintMode} 
              ...
              <div class="accordion-content" transition:slide={{ duration: 200 }}>
                {#each foundationSections as section}
                  {#if data?.sections?.[section]}
                    ...
                    >
                      <span class="nav-icon">{sectionEmojis[section] || '📄'}</span>
      ...
      {#each sectionsToShow as section (section)}
          ...
          <div class="section-content" id={section}>
            ...
            {#if section === 'executive-summary-for-the-skeptic' && !isPrintMode && translationsReady}
              <div class="guide-navigation">
                ...
                <button class="primary-btn" on:click={() => setActiveSection('introduction-context')}>
        ...
        {:else}
          <div class="missing-section">
            <h2>{getTextWithFallback('drrFramework.errors.sectionNotFound', `Section "${section}" not found`).replace('{section}', section)}</h2>
            <p>{getTextWithFallback('drrFramework.errors.contentInDevelopment', 'This content is still being developed.')}</p>
          </div>
        {/if}
      {/each}
    </div>
...
{:else}
  ...
  <div class="loading-container">
    ...
    <p>{getTextWithFallback('drrFramework.loading.text', 'Loading framework content...')}</p>
  </div>
{/if}

<style>
  /* Disaster Risk Reduction & Resilience color scheme */
  :root {
    --drr-primary: #B91C1C;        /* Deep emergency red */
    --drr-secondary: #059669;       /* Resilient green */
    --drr-accent: #D97706;          /* Warning amber */
    --drr-success: #10B981;         /* Recovery green */
    --drr-warning: #F59E0B;         /* Alert orange */
    --drr-danger: #DC2626;          /* Critical red */
    --drr-light: #FEF3C7;           /* Warm cream */
    --drr-dark: #7C2D12;            /* Deep earth */
    --drr-wisdom: #374151;          /* Steady gray */
    --drr-earth: #A16207;           /* Rich earth tone */
  }
  ...
  .loading-spinner {
    ...
    border-top: 4px solid var(--drr-primary);
    border-radius: 50%;
  ...
  .section-nav {
    ...
    background: linear-gradient(to bottom, #fef9f5, #fef3c7);
    border-radius: 0.5rem;
    padding: 1rem;
  }
  ...
  .accordion-header:focus-visible {
    outline: 2px solid var(--drr-accent);
    outline-offset: 2px;
  ...
  .nav-item.active:hover {
    background-color: var(--drr-secondary);
  }

  .overview-item {
    background: linear-gradient(135deg, rgba(185, 28, 28, 0.1), rgba(5, 150, 105, 0.1));
  ...
  .progress-indicator {
    margin-bottom: 2rem;
    padding: 1rem;
    background: linear-gradient(90deg, rgba(185, 28, 28, 0.1), rgba(5, 150, 105, 0.1));
    border-radius: 0.5rem;
  ...
  .progress-fill {
    height: 100%;
    background: linear-gradient(90deg, var(--drr-primary), var(--drr-secondary));
    border-radius: 4px;
  ...
  /* DRR guide card */
  .drr-guide-card {
    background: linear-gradient(135deg, rgba(5, 150, 105, 0.1) 0%, rgba(185, 28, 28, 0.1) 100%);
  ...
  .content :global(blockquote) {
    background-color: rgba(5, 150, 105, 0.1);
    border-left: 4px solid var(--drr-secondary);
    padding: 1rem 1.5rem;
    margin: 1.5rem 0;
    border-radius: 0.5rem;
  }
