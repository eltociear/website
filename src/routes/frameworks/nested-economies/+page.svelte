<!-- src/routes/frameworks/nested-economies/+page.svelte -->
<script>
 ...
 // Extract Nested Economies translations for shorter references
 ...
 // Define section groupings - match the Nested Economies structure
 const sectionGroups = {
   overview: ['preamble', 'index', 'at-a-glance', 'executive-summary-for-the-skeptic'],
   core: ['introduction', 'core-principles', 'implementation-strategies', 'resource-disparities', 'governance-integration'],
   implementation: ['timeline', 'monitoring', 'engagement'],
   resources: ['conclusion', 'appendices']
 };

 // Section icons - tailored to economic integration theme
 const sectionIcons = {
   // Overview cluster
   'preamble': '📜',
   'index': '💱',
 ...
 function getSectionTitle(section) {
   return nestedEconomies.sections?.[section] || section;
 }

 // Get section category titles using short references
 function getSectionCategoryTitle(category) {
   return nestedEconomies.categories?.[category] || category;
 }

 // Function to get shortened section titles for navigation using short references
 function getShortSectionTitle(section) {
   return nestedEconomies.sectionsShort?.[section] || getSectionTitle(section);
 }

 // Function to download the Nested Economies guide PDF
 function downloadNestedEconomiesGuide(version = '') {
   const versionSuffix = version ? `-${version}` : '';
   const pdfUrl = `${base}/assets/pdf/nested-economies-framework${versionSuffix}-${currentLocale}.pdf`;
   const link = document.createElement('a');
   link.href = pdfUrl;
   link.download = `nested-economies-framework${versionSuffix}.pdf`;
 ...
 $: quickStartGuides = nestedEconomies.quickStart?.guides || [
   {
     id: 'at-a-glance',
     icon: '⚡',
     title: 'At a Glance',
     description: '5-minute overview of nested economic sovereignty'
   },
   {
     id: 'executive-summary-for-the-skeptic',
     icon: '🤔',
     title: 'For Skeptics',
     description: 'Evidence-based case for community-controlled economics'
   }
 ];
...
<svelte:head>
 <title>{nestedEconomies.meta?.title || 'Nested Economies: Economic Integration Framework - Global Governance Framework'}</title>
 <meta name="description" content="{nestedEconomies.meta?.description || 'A framework for building economies of Right Relationship through community sovereignty, Hearts currency, and regenerative development'}" />
</svelte:head>
...
{#if mounted}
 <div class="documentation-container">
   ...
   <div class="content">
     <!-- Quick Access Card for Nested Economies Quick Start -->
     {#if !isPrintMode && !isQuickStartActive && (activeSection === 'index' || activeSection === 'preamble')}
       <div class="nested-economies-guide-card">
         <div class="card-content">
           <div class="card-icon">💱</div>
           <div class="card-text">
             <h3>{nestedEconomies.quickStart?.new || 'New to Economic Integration?'}</h3>
             <p>{nestedEconomies.quickStart?.start || 'Start with our quick guides that explain community-controlled economic systems for different audiences.'}</p>
           </div>
           <div class="card-actions">
             <div class="dropdown">
               <button 
                 ...
                 type="button"
               >
                 {nestedEconomies.quickStart?.button || 'Choose Guide'} 
     ...
     <!-- Sub-navigation for Nested Economies framework sections -->
     ...
     {#if !isPrintMode && isCoreSection && allCoreSections.length > 0}
       <div class="progress-indicator">
         ...
         <span class="progress-text">{nestedEconomies.progress?.text?.replace('{current}', allCoreSections.indexOf(activeSection) + 1).replace('{total}', allCoreSections.length) || `Section ${allCoreSections.indexOf(activeSection) + 1} of ${allCoreSections.length}`}</span>
       </div>
     {/if}
     ...
     {#each sectionsToShow as section (section)}
           ...
           {#if !isPrintMode && data.sectionsUsingEnglishFallback?.includes(section)}
             <div class="language-fallback-notice">
               ...
               <div class="notice-content">
                 <strong>{nestedEconomies.languageFallback?.title || 'Content in your language coming soon'}</strong>
                 <p>{nestedEconomies.languageFallback?.description || 'This section is currently displayed in English until translation is complete.'}</p>
               </div>
             </div>
           {/if}
           ...
           {#if allQuickStartSections.includes(section) && section !== 'index' && section !== 'preamble'}
             ...
             {#if !isPrintMode}
               <div class="guide-selector">
                 <h2>{nestedEconomies.quickStart?.title || 'Nested Economies Quick Start'}</h2>
                 <p>{nestedEconomies.quickStart?.description || 'Choose the version that best matches your needs:'}</p>
             ...
             {#if !isPrintMode}
               <div class="guide-navigation">
                 <button class="secondary-btn" on:click={() => downloadNestedEconomiesGuide(section.replace('-', ''))}>
                   {nestedEconomies.quickStart?.download || 'Download PDF Version'} <span class="download-icon">↓</span>
                 </button>
                 <button class="primary-btn" on:click={() => setActiveSection('introduction')}>
                   {nestedEconomies.quickStart?.continue || 'Continue to Full Framework'} <span class="arrow-icon">→</span>
                 </button>
               </div>
             {/if}
           {:else}
           ...
           {#if activeSection === 'preamble' && !isPrintMode}
             <div class="preamble-navigation">
               <button class="primary-btn" on:click={() => setActiveSection('index')}>
                 {nestedEconomies.preamble?.continue || 'Continue to Framework Overview'} 
             ...
             <div class="section-navigation">
               {#if allCoreSections.indexOf(activeSection) > 0}
                 <button class="nav-btn prev-btn" on:click={() => {
                   ...
                   ← {nestedEconomies.navigation?.previousSection || 'Previous Section'}
                 </button>
               {/if}
               ...
               {#if allCoreSections.indexOf(activeSection) < allCoreSections.length - 1}
                 <button class="nav-btn next-btn" on:click={() => {
                   ...
                   {nestedEconomies.navigation?.nextSection || 'Next Section'} →
                 </button>
               {/if}
             </div>
           {/if}
       ...
       {:else}
         <div class="missing-section">
           <h2>{nestedEconomies.errors?.sectionNotFound?.replace('{section}', section) || `Section "${section}" not found`}</h2>
           <p>{nestedEconomies.errors?.contentInDevelopment || 'This content is still being developed.'}</p>
         </div>
       {/if}
     {/each}
   </div>
...
{:else}
 ...
 <div class="loading-container">
   ...
   <p>{nestedEconomies.loading?.text || 'Loading economic integration framework...'}</p>
 </div>
{/if}

<style>
 /* Nested Economies color scheme - Economic Integration & Community themed */
 :root {
   --nested-primary: #2E7D32; /* Forest green for growth and sustainability */
   --nested-secondary: #4CAF50; /* Brighter green for prosperity */
   --nested-accent: #FF6F00; /* Orange for Hearts currency */
   --nested-hearts: #E91E63; /* Pink/red for Hearts */
   --nested-leaves: #66BB6A; /* Leaf green for ecological tokens */
   --nested-community: #1976D2; /* Blue for community coordination */
   --nested-warning: #FF9800; /* Amber for caution */
   --nested-danger: #D32F2F; /* Red for risks */
   --nested-light: #E8F5E8; /* Very light green */
   --nested-dark: #1B5E20; /* Dark green */
   --nested-cooperation: #4FC3F7; /* Light blue for cooperation */
   --nested-autonomy: #AB47BC; /* Purple for local autonomy */
 }
 ...
 .nav-item.active:hover {
   background-color: var(--nested-secondary);
 }
 ...
 .featured-item.active {
   background: var(--nested-primary);
   border-color: var(--nested-primary);
 }
 ...
 .nested-economies-guide-card .dropdown-menu {
   position: absolute;
   top: 100%;
   left: 0;
   z-index: 1001;
 ...
 .primary-btn:hover {
   background-color: var(--nested-dark);
 ...
 .secondary-btn {
   ...
   color: var(--nested-primary);
   border: 1px solid var(--nested-primary);
   padding: 0.5rem 1rem;
   border-radius: 0.375rem;
   font-weight: 500;
   cursor: pointer;
 ...
 .secondary-btn:hover {
   background-color: var(--nested-light);
   transform: translateY(-1px);
 }
 ...
 .progress-indicator {
   ...
   border-radius: 0.5rem;
   border-left: 4px solid var(--nested-primary);
 }
 ...
 .progress-fill {
   height: 100%;
   background: linear-gradient(90deg, var(--nested-primary), var(--nested-secondary));
   border-radius: 4px;
 ...
 :global(.content thead) {
   background: linear-gradient(to right, var(--nested-primary), var(--nested-secondary));
 }
 ...
 :global(.content th) {
   ...
   color: #ffffff;
   border: none;
   border-bottom: 2px solid var(--nested-primary);
 }
 ...
 :global(.content table tfoot) {
   ...
   color: var(--nested-primary);
   text-align: left;
   border-top: 1px solid var(--nested-primary);
 }
 ...
 .loading-spinner {
   width: 3rem;
   height: 3rem;
   border: 3px solid var(--nested-light);
   border-top: 3px solid var(--nested-primary);
   border-radius: 50%;
 ...
 .nav-item[data-section="preamble"] {
   ...
   border-left: 3px solid var(--nested-primary);
 }
</style>
