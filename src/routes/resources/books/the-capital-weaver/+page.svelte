<!-- src/routes/resources/books/the-capital-weaver/+page.svelte -->
<script>
  ...
  $: bf = translationsReady ? ($t('booksCapitalWeaver') || {}) : {};
  ...
  $: frontMatterSections = ['book-cover', 'quick-reference', 'how-to-use-this-book', 'table-of-contents', 'preface', 'introduction'];
  $: backMatterSections = ['glossary', 'further-reading', 'appendix-a', 'appendix-b', 'appendix-c', 'appendix-d', 'appendix-e', 'appendix-f', 'appendix-g', 'appendix-h', 'appendix-i', 'appendix-j', 'appendix-k', 'about'];
  ...
  onMount(async () => {
    ...
    if (browser) {
      ...
      if (window.location.pathname !== '/resources/books/the-capital-weaver') {
        ...
        const correctUrl = `/resources/books/the-capital-weaver${window.location.search}${extractedHash}`;
        window.history.replaceState(null, '', correctUrl);
      }
      
      if (!translationsReady) {
        try {
          await loadTranslations($locale, '/resources/books/the-capital-weaver');
        } catch (e) {
  ...
  function setActiveSection(section) {
    ...
    if (browser) {
      const newUrl = `/resources/books/the-capital-weaver${window.location.search}#${section}`;
  ...
  function downloadBook() {
    const pdfUrl = `/static/resources/The_Capital_Weaver_-_A_Practical_Guide_to_Regenerative_Investing.pdf`;
    const link = document.createElement('a');
    link.href = pdfUrl;
    link.download = `The_Capital_Weaver.pdf`;
...
<svelte:head>
  <title>{getTextWithFallback('booksCapitalWeaver.meta.title', 'The Capital Weaver - A Practical Guide to Regenerative Investing')}</title>
  <meta name="description" content="{getTextWithFallback('booksCapitalWeaver.meta.description', 'A comprehensive guide to regenerative investing that moves beyond ESG to create prosperity through healing rather than extraction.')}" />
</svelte:head>
      ...
      <div class="container">
        <div class="breadcrumb-nav">
          ...
          <span class="breadcrumb-current">
            {bf.breadcrumb?.currentBook || 'The Capital Weaver'}
          </span>
        </div>
      </div>
    </div>
    ...
    {#if !isPrintMode}
      <div class="book-nav">
        <div class="book-header">
          ...
          <div class="book-info">
            <h2 class="book-title">{bf.bookTitle || 'The Capital Weaver'}</h2>
            <p class="book-subtitle">{bf.bookSubtitle || 'A Practical Guide to Regenerative Investing'}</p>
          </div>
        </div>
        ...
        <div class="nav-sections">
          ...
          {#each Object.entries(bookParts) as [partName, chapters], partIndex}
            {@const partOpen = [part1Open, part2Open, part3Open][partIndex]}
            {@const togglePart = [togglePart1, togglePart2, togglePart3][partIndex]}
      ...
      {#each sectionsToShow as section (section)}
        ...
        {:else}
          <div class="missing-section">
            <h2>{getTextWithFallback('booksCapitalWeaver.errors.sectionNotFound', `Section "${section}" not found`).replace('{section}', section)}</h2>
            <p>{getTextWithFallback('booksCapitalWeaver.errors.contentInDevelopment', 'This content is still being developed.')}</p>
          </div>
        {/if}
      {/each}
    </div>
...
{:else}
  ...
  <div class="loading-container">
    ...
    <p>{getTextWithFallback('booksCapitalWeaver.loading.text', 'Loading book content...')}</p>
  </div>
{/if}

<style>
  /* Capital Weaver Book color scheme */
  :root {
    --book-primary: #2D5016;
    --book-secondary: #4A7C59;
    --book-accent: #7FB069;
    --book-success: #A7C957;
    --book-warning: #F2CC8F;
    --book-danger: #E07A5F;
  ...
  .progress-indicator {
    margin-bottom: 2rem;
    padding: 1rem;
    background: linear-gradient(90deg, rgba(45, 80, 22, 0.1), rgba(127, 176, 105, 0.1));
    border-radius: 0.5rem;
  ...
  .content-language-notice {
    background: linear-gradient(135deg, rgba(242, 204, 143, 0.1), rgba(167, 201, 87, 0.1));
    border: 1px solid rgba(242, 204, 143, 0.3);
    border-radius: 0.75rem;
    padding: 1.5rem;
    margin-bottom: 2rem;
    box-shadow: 0 2px 4px rgba(242, 204, 143, 0.1);
  }
